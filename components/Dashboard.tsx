...
import { MenuSpecialsCard } from "./MenuSpecialsCard";
...
export function Dashboard({ initial }: { initial: Briefing }) {
  ...
  return (
    <div className="min-h-screen bg-seine-bg">
      <Header
        briefing={briefing}
        mode={mode}
        onMode={setMode}
      ...
      <main className="mx-auto max-w-6xl space-y-4 px-4 py-5">
        ...
        <MenuSpecialsCard briefing={briefing} />
        ...
        <footer className="pb-8 pt-1 text-center text-[11px] text-seine-muted">
          Briefing « Glaces en Seine » · Quai de Seine · Bon service ⚓
        </footer>
      </main>
    </div>
  );
